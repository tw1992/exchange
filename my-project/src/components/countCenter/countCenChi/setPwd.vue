<template>
  <div>
    <contenthead :step="[true,true,false,true]" :stepIndex="2"></contenthead>
    <div class="setPwd">
      <certifytitle :title="tiptitle"></certifytitle>
      <form action="" class="formStyleEditPwd">
        <div v-if="!bind">
          <!--<label for="">-->
          <!--&lt;!&ndash;<input type="password" placeholder="请输入原密码" v-model="pwd" @input="change('pwd')">&ndash;&gt;-->
          <!--</label>-->
          <label for="">
        ...
        <div class="editpwdClass" v-if="bind">
          <el-button type="primary"  @click="editPwd">修改交易密码</el-button>
        </div>
      </form>
  ...
  export default{
    data(){
      return {
        ...
        tiptitle:'您尚未绑定手机，绑定手机能更好保障账户安全',
        tip:'6位密码，仅支持数字',
        readyOnly:true,
        disable:true,
        bind:false,
    ...
    methods:{
      change(type){
        ...
        if(this.newrepwd && this.suerNewrepwd){
          this.disable = false;
        }
      },
      subPwd(){
      ...
      userType(){
        ...
        this.interFace('user_type').then(function(res){   //status 1:验证通过   0不通过
          ...
          if(res.pwdtrade == 1){
            ...
            that.bind = false;
            that.tiptitle = '您尚未绑定手机，绑定手机能更好保障账户安全';
          }
        })
      }
    },
